...
def main():
    ...
    processed_df = get_combined_df(
        movements_df, events_df, get_blank_df(event_types, locations)
    )

    selected_los = events_df[events_df.Event_Type == "Loss of Separation"]
    selected_fac = events_df[
        (events_df.Location == "Red City")
        & (events_df.month == "2022-05")
        & (events_df.Event_Type == "Facility Issue")
    ]
