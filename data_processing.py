...
OUT_PATH = os.path.join(DATA_DIR, r"processed_data.pkl")
...
def get_movements_data(year: int = 2022) -> pd.DataFrame:
    ...
    movements_df.datetime = pd.to_datetime(
        movements_df.datetime, format="%d/%m/%Y %H:%M:%S"
    )
    movements_df = (
        movements_df[movements_df.datetime.dt.year == year]
        .copy()
...
def get_events_data(year: int = 2022) -> pd.DataFrame:
    ...
    events_df = (
        events_df[events_df.Event_Date.dt.year == year].copy().reset_index(drop=True)
    )
...
def get_combined_df(
    movements_df: pd.DataFrame, events_df: pd.DataFrame, blank_df: pd.DataFrame
) -> pd.DataFrame:
    # Get the events for this city
    events_df = (
        events_df.groupby(["Event_Type", "month", "Location"])
        .count()
        .iloc[:, [0]]
    ...
    res_df = pd.merge(
        blank_df, events_df, on=["month", "Event_Type", "Location"], how="left"
    )
    ...
    # Merge the city-specific with the 'All Cities' df and return it for analysis steps
...
if __name__ == "__main__":
    ...
    df = get_combined_df(movements_df, events_df, get_blank_df(event_types, locations))
